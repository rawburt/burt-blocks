...
static void renderText(SDL_Renderer *renderer, TTF_Font *font, const char *text, const int x, const int y, const int align = ALIGN_LEFT)
{
...
SDL_Texture *loadTexture(SDL_Renderer *renderer, const char *file_path, const int alpha)
{
...
void initUI(ui_state & ui)
{
    ...
    ui.textures[TEXTURE_CELLG] = loadTexture(ui.renderer, "./assets/cell.png", 125);
...
void render(ui_state & ui, const game_state & game)
{
    ...
    case WAITING:
        {
            renderText(ui.renderer, ui.fonts[FONT_TITLE], "BURT BLOCKS", 0, 200, ALIGN_CENTER);
            renderText(ui.renderer, ui.fonts[FONT_MAIN], "Press R to play.", 0, 290, ALIGN_CENTER);
        }
        break;
    case PLAYING:
        {
            ...
            renderText(ui.renderer, ui.fonts[FONT_MAIN], ui.score_text, 30, 30, ALIGN_RIGHT);
        }
        break;
    case OVER:
        {
            renderText(ui.renderer, ui.fonts[FONT_TITLE], "GAME OVER", 0, 200, ALIGN_CENTER);
            renderText(ui.renderer, ui.fonts[FONT_MAIN], "Press R to restart.", 0, 290, ALIGN_CENTER);
        }
        break;
    }
